...
cp ./testresult.txt ./testresult.sh
cat testresult.sh
n=$(wc -l testresult.sh)
echo
...
ee=$end
x=$((end-start))
sed -i -e 's#^#\"\./m3u8-download #' testresult.sh > download-command.txt
sed -i -e ''$s','$ee'!d' testresult.sh > download-command.txt
read -p "Give the video name: " vname
sed -i -e 's/$/ '/videos/$vname'/' testresult.sh > downlaod-command.txt
cat testresult.sh
echo
read -p "Waiting..., please check if the curlresult with video name is correct: " waitingi1
for (( i=1; i<=x+1; i++ )); do 
	echo i
	sed -i -e ''$i' s/$/'$i'\"/' testresult.sh > download-command.txt
	exec cat testresult.sh | grep \.\/m3u8
done

echo "You are going to download from source #"$((start+1))
command=$(cat testresult.sh | grep \.\/m3u8)
echo
